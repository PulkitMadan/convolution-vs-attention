...
def shape_bias(model, dataloaders, class_map=mapping_207_reverse):
    ...
    correct_pred_both = \
        shape_bias_df[(shape_bias_df['correct_shape'] == True) | (shape_bias_df['correct_texture'] == True)].count()[
            'pred']
    ...
    fraction_correct_texture = len(shape_bias_df_nc.loc[shape_bias_df_nc.correct_texture == True]) / len(
        shape_bias_df_nc)
        ...
        for i, (inputs, labels) in enumerate(dataloaders[select]):
            ...
            for j in range(inputs.size()[0]):
                ...
                if (pred_name == lab_name) and (tex_name != lab_name) and (lab_cor == 0):
                    ...
                    nl = '\n'
                    ax.set_title(
                        f'Predicted: {pred_name}, ID: {pred_nameid} {nl} True: {lab_name}, ID: {lab_nameid} {nl} Texture: {tex_name}, ID: {tex_nameid}')
